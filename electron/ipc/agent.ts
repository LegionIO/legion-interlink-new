...
import { resolveModelForThread, resolveModelCatalog, type ModelCatalogEntry } from '../agent/model-catalog.js';
...
/** Hot-swap MCP tools without touching built-in or skill tools */
export function updateMcpTools(mcpTools: ToolDefinition[]): void {
  // MCP tool names contain ':' but NOT 'skill:' prefix
  const nonMcp = registeredTools.filter((t) => !t.name.includes(':') || t.name.startsWith('skill:'));
